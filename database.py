...
def update_resource_status(resource_id: int, status: str, consecutive_failures: int):
    ...
    with _lock:
        ...
        if old_status != status:
            ...
            if status == "down":
                # Opened new downtime event
                cursor.execute('''
                    INSERT INTO downtime_events (resource_id, went_down_at, went_up_at)
                    VALUES (?, ?, NULL)
                ''', (resource_id, now))
